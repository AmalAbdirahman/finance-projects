...
class MinHeap:
    ...
    def _get_node_at(self, ix):
        """Retrieve the node at the given index in the heap.

        Args:
            ix (int): The index of the desired node.

        Returns:
            TreeNode: The node at the given index.

        Raises:
            KeyError: If the index is invalid or out of bounds.
        """
        if ix <= 0 or ix > self._size:
            raise KeyError(f"Index {ix} is out of bounds for the heap.")

        # Convert index to binary and drop the leading '1'
        path = bin(ix)[3:]  # Skip '0b1'
        current = self.root

        # Traverse the tree based on the binary path
        for direction in path:
            ...
            if current is None:
                raise KeyError(f"Index {ix} does not exist in the heap.")

        return current

    def _add_leaf(self, leaf):
        """Add a new leaf node at the first available position in the heap.

        Args:
            leaf (TreeNode): The new leaf node to add.

        Returns:
            TreeNode: The newly added leaf node.
        """
        self._size += 1
        if self._size == 1:
            self.root = leaf
        ...
        parent_index = self._size // 2
        parent = self._get_node_at(parent_index)
    ...
    def insert_node(self, node):
        ...
        new_leaf = self._add_leaf(node)
        self._sift_up(new_leaf)
        return new_leaf

    def _bubble_up(self, node):
        """Restore the heap property by bubbling up the node."""
        while node.parent and node.key < node.parent.key:
            self._swap_node_with_parent(node)
    ...
    def insert(self, key, value=None):
        ...
        Args:
            ...
            value (Any): The value of the new node (default is None).
        ...
        if self.root is None:
            ...
            parent = self._get_node_at((self._size + 1) // 2)
    ...
    def extract(self):
        """Remove and return the root node (minimum key) from the heap.
        ...
        self._replace_node_with_leaf(root, last_leaf)
    ...
    def delete_node(self, node):
        """Deletes the specified node from the heap.
                ...
                The node is replaced with the last leaf, and the heap is restructured.

                Args:
                    node (TreeNode): The node to delete.
        ...
        if self._size == 1 and node == self.root:
            self.root = None
            self._size -= 1
            return node
        ...
        if node == last_leaf:
            if last_leaf.parent:
                if last_leaf.parent.left == last_leaf:
                    last_leaf.parent.left = None
                else:
                    last_leaf.parent.right = None
            self._size -= 1
            return node

        self._replace_node_with_leaf(node, last_leaf)

        parent = last_leaf.parent
        if parent and last_leaf.key < parent.key:
